...
int main (int argc, char **argv) {
    ...
    for(auto translation:translations){
        ...
        while(!q.empty()){
            ...
            if(dir==0){
                hGlobalCoordinatesY[node]-=amount;
                // up
            }else if(dir==1){
                //down
                hGlobalCoordinatesY[node]+=amount;
                ...
                //left
                hGlobalCoordinatesX[node]-=amount;
            }else{
                //right
                hGlobalCoordinatesX[node]+=amount;
            }
            for(int child = start;child<end;child++){
                int childNode = hCsr[child];
                q.push(childNode);
            }
        }  
              
    }
    vector<vector<int>> dup(frameSizeX,vector<int>(frameSizeY,0));
    for(int i=0;i<V;i++)
    {
        int r = hFrameSizeX[i];
        int c = hFrameSizeY[i];
        int s = hGlobalCoordinatesX[i];
        int e = hGlobalCoordinatesY[i];
        for(int j=s;j<s+c;j++)
        {
            for(int k=e;k<e+r;k++)
            {
                if(j>=0 && k>=0 && j<frameSizeX && k<frameSizeY && dup[j][k]<=hOpacity[i])
                {
                    dup[j][k] = hOpacity[i];
                    hFinalPng[j*frameSizeX + k] = hMesh[i][(j-s)*r+(k-e)];
                }
            }
        }
    }
    for(int i=0;i<frameSizeX;i++)
    {
        for(int j=0;j<frameSizeY;j++)
        {
            printf("%d",hFinalPng[i*frameSizeX + j]);
        }
        printf("\n");
    }
